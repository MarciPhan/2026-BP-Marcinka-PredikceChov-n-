...
# need all intents for proper tracking
intents = discord.Intents.all()
bot = commands.Bot(command_prefix=config.BOT_PREFIX, intents=intents)

# remove default help, we have custom one
bot.remove_command("help")

pending_console_msgs: list[str] = []

async def send_console_log(msg: str):
    """Send log to console channel or buffer if bot not ready yet"""
    fullmsg = f"{ts()} {msg}"
    try:
        if not bot.is_ready():
            # bot not ready yet, save for later
            pending_console_msgs.append(fullmsg)
            print(f"[PENDING] {fullmsg}")
            return
        channel = bot.get_channel(config.CONSOLE_CHANNEL_ID)
        print(f"[lOG] {fullmsg}")  # typo but whatever, works
        if channel:
